...
```{r nsSNP_AF_hist, eval=TRUE, message=FALSE, echo=FALSE, fig.width=9, fig.height=4.5}
library(gridExtra)
...
#to_plot$EAC_bin <- cut(to_plot$EAC, breaks = breaks)
#to_plot<-merge(to_plot, list_goi[c("Gene", "mem")])
#to_plot$mem <- to_plot$mem=="CGC"
#p1 <- ggplot(aes(EAC_bin, fill=Clinvar), data=subset(to_plot, !is.na(pred_patho))) + theme_few() + theme(legend.position="top") + geom_bar(stat="bin") 
p1 <- ggplot(aes(EAC, fill=Origin), data=to_plot) + theme_few() + stat_bin(right=F) + scale_x_sqrt(breaks=breaks)
p1 <- p1 + theme(axis.text= element_text(hjust=0.5), legend.position = c(.8, .85) ) +  scale_fill_brewer(palette="Set2")
p1 <- p1 + xlab("AC")

cols <- c("Benign"="#e78ac3","Deleterious"="#a6d854")
...
print(c("minimal cnv.pval", signif(min(lowfq@VAR$cnv.pval), 2)))
...
```{r nsSNP_rr_cgc_hist, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=8, fig.height=4.5}
to_plot <- subset(nonsyn_VARu_rr, mem=="CGC")
genes <- unique(droplevels(subset(to_plot, pred_patho!="Benign"))$Gene)
to_plot <- subset(to_plot, Gene %in% genes)
genes <- table(to_plot$Gene)
to_plot$Gene <- factor(to_plot$Gene, levels=names(sort(-genes)))
p <- ggplot(aes(Gene, fill=Origin), data= to_plot) + geom_bar() + theme_few() +facet_grid(pred_patho~.)
